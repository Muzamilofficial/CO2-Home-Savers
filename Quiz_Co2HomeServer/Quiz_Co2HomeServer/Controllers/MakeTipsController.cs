...
{
    public class MakeTipsController : Controller
    {
        db_Co2HomeSaversEntities1 db = new db_Co2HomeSaversEntities1();
        public async Task<ActionResult> R_Index()
        {
            var data = db.Database.SqlQuery<GetWattsRecords_Result>("GetWattsRecords @username", new SqlParameter("@username", "1")).ToList();
