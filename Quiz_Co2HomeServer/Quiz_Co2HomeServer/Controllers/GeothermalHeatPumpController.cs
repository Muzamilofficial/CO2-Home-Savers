...
{
    public class GeothermalHeatPumpController : Controller
    {
        // GET: GeothermalHeatPump
        ...
        public JsonResult InsertRefrigilator(string username, int noOfRefrigilators)
        {
            try
            {
                ...
                var existingRefrigerator = dbContext.tbl_Electriccarcharger.FirstOrDefault(r => r.Usernme == username);

                if (existingRefrigerator != null)
                {
                    // If a record with the username exists, update the NoOfRefrigilators value
                    existingRefrigerator.NoOfElectriccarchargers = noOfRefrigilators;
                }
                else
                {
                    // If the username doesn't exist, create a new record
                    var refrigerator = new tbl_Electriccarcharger
                    {
                        CategoryID = 6,
                        Appliance = "Electric car charger",
                        Usernme = username,
                        NoOfElectriccarchargers = noOfRefrigilators
                        // Set other properties to null or default values if necessary
                    };

                    // Add the new entity to the context
                    dbContext.tbl_Electriccarcharger.Add(refrigerator);
                }
        ...
        {
            public int NoOfElectriccarchargers { get; set; }
        }
        ...
        public ActionResult UpdateMakes(List<string> makes)
        {
            ...
            if (usernameModel != null)
            {
                ...
                {
                    var refrigerator = dbContext.tbl_Electriccarcharger.FirstOrDefault(r => r.Usernme == username);
        ...
        public async Task<ActionResult> Question2()
        {
            try
            {
                ...
                {
                    string username = usernameModel.Username;
                    var result = dbContext.Database.SqlQuery<RefrigeratorResult>("exec SelectElectriccarchargerByUsernme @Username",
                    new SqlParameter("@Username", username)).FirstOrDefault();

                    if (result != null)
                    {
                        ViewBag.NoOfRefrigilators = result.NoOfElectriccarchargers;
                    }
                    else
                    {
                ...
                {
                    Prompt = "show 20 asian names of model of Electric car charger along there companies",
        ...
        public async Task<List<string>> GetRefrigeratorNamesAsync()
        {
            ...
            var completionRequest = new CompletionRequest
            {
                Prompt = "Top 20 Electric car charger companies",
                Model = Model.DavinciText,
                MaxTokens = 200,
            };
        ...
        public async Task<List<string>> GetRefrigeratorNamesAsyncn()
        {
            ...
            var make1 = dbContext.tbl_Electriccarcharger.Where(r => r.Usernme == username).Select(r => r.Make1).FirstOrDefault();

            var completionRequest = new CompletionRequest
            {
                Prompt = "Generate 20 Model Of" + make1 + "Electric car chargers ",
                Model = Model.DavinciText,
                MaxTokens = 200,
            };
        ...
        public async Task<List<string>> GetRefrigeratorNamesAsync2()
        {
            ...
            var make2 = dbContext.tbl_Electriccarcharger.Where(r => r.Usernme == username).Select(r => r.Make2).FirstOrDefault();

            var completionRequest = new CompletionRequest
            {
                Prompt = "Generate 20 Model Of" + make2 + "Electric car chargers ",
                Model = Model.DavinciText,
                MaxTokens = 200,
            };
        ...
        public async Task<List<string>> GetRefrigeratorNamesAsync3()
        {
            ...
            var make3 = dbContext.tbl_Electriccarcharger.Where(r => r.Usernme == username).Select(r => r.Make3).FirstOrDefault();

            var completionRequest = new CompletionRequest
            {
                Prompt = "Generate 20 Model Of" + make3 + "Electric car chargers ",
                Model = Model.DavinciText,
                MaxTokens = 200,
            };
        ...
        public async Task<ActionResult> Question3()
        {
            ...
            try
            {
                ...
                if (usernameModel != null)
                {
                    ...
                    {
                        ViewBag.NoOfRefrigilators = result.NoOfElectriccarchargers;
                        if (result.NoOfElectriccarchargers == 2)
                        {
                        ...
                        if (result.NoOfElectriccarchargers == 3)
                        {
                            var newOptions = new List<string> { "Option X", "Option Y", "Option Z" }; // Replace with your actual new options
            ...
            if (usernameModel != null)
            {
                using (var dbContext = new db_Co2HomeSaversEntities1())
                {
                    ...
                    if (refrigerator != null)
                    {
                        ...
                        dbContext.Database.ExecuteSqlCommand("EXEC UpdateTotalWattsForElectriccarcharger @Username", new SqlParameter("@Username", username));
                    }
                }
            }
        ...
        public async Task<ActionResult> Tips()
        {
            try
            {
                ...
                {
                    Prompt = "Generate Energy saving only 1 tip for Electric car charger without numbering and special charactors",
                    Model = Model.DavinciText,
        ...
        public ActionResult UpdateTips(string AffilaiteLink, string ImageAddress, tbl_Refrigilator model)
        {
            ...
            return RedirectToAction("Index", "healthiestlightbulbs");
        }
