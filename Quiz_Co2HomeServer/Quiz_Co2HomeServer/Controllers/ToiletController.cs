    ...
    public class ToiletController : Controller
    {
        ...
        public JsonResult InsertRefrigilator(string username, int noOfRefrigilators)
        {
            try
            {
                ...
                var existingRefrigerator = dbContext.tbl_Toilet.FirstOrDefault(r => r.Usernme == username);

                if (existingRefrigerator != null)
                {
                    // If a record with the username exists, update the NoOfRefrigilators value
                    existingRefrigerator.NoOfToilets = noOfRefrigilators;
                }
                else
                {
                    // If the username doesn't exist, create a new record
                    var refrigerator = new tbl_Toilet
                    {
                        CategoryID = 4,
                        Appliance = "Toilet",
                        Usernme = username,
                        NoOfToilets = noOfRefrigilators
                        // Set other properties to null or default values if necessary
                    };

                    // Add the new entity to the context
                    dbContext.tbl_Toilet.Add(refrigerator);
                }
        ...
        public ActionResult UpdateMakes(List<string> makes)
        {
            ...
            if (usernameModel != null)
            {
                ...
                {
                    var refrigerator = dbContext.tbl_Toilet.FirstOrDefault(r => r.Usernme == username);
        ...
        public async Task<ActionResult> Question2()
        {
            try
            {
                ...
                {
                    string username = usernameModel.Username;
                    var result = dbContext.Database.SqlQuery<RefrigeratorResult>("exec SelectToiletByUsernme @Username",
                    new SqlParameter("@Username", username)).FirstOrDefault();

                    if (result != null)
                    {
                        ViewBag.NoOfRefrigilators = result.NoOfToilets;
                    }
                    else
                    {
                ...
                {
                    Prompt = "show 20 asian names of model of Electric Toilet along there companies",
        ...
        public async Task<List<string>> GetRefrigeratorNamesAsync()
        {
            ...
            var completionRequest = new CompletionRequest
            {
                Prompt = "Top 20 Electric Toilet companies",
                Model = Model.DavinciText,
                MaxTokens = 200,
            };
        ...
        public async Task<List<string>> GetRefrigeratorNamesAsyncn()
        {
            ...
            var make1 = dbContext.tbl_Toilet.Where(r => r.Usernme == username).Select(r => r.Make1).FirstOrDefault();

            var completionRequest = new CompletionRequest
            {
                Prompt = "Generate 20 Model Of" + make1 + "Electric Toilets ",
                Model = Model.DavinciText,
                MaxTokens = 200,
            };
        ...
        public async Task<List<string>> GetRefrigeratorNamesAsync2()
        {
            ...
            var make2 = dbContext.tbl_Toilet.Where(r => r.Usernme == username).Select(r => r.Make2).FirstOrDefault();

            var completionRequest = new CompletionRequest
            {
                Prompt = "Generate 20 Model Of" + make2 + "Electric Toilets ",
                Model = Model.DavinciText,
                MaxTokens = 200,
            };
        ...
        public async Task<List<string>> GetRefrigeratorNamesAsync3()
        {
            ...
            var make3 = dbContext.tbl_Toilet.Where(r => r.Usernme == username).Select(r => r.Make3).FirstOrDefault();

            var completionRequest = new CompletionRequest
            {
                Prompt = "Generate 20 Model Of" + make3 + "Electric Toilets ",
                Model = Model.DavinciText,
                MaxTokens = 200,
            };
            ...
            if (usernameModel != null)
            {
                using (var dbContext = new db_Co2HomeSaversEntities1())
                {
                    ...
                    if (refrigerator != null)
                    {
                        ...
                        dbContext.Database.ExecuteSqlCommand("EXEC UpdateTotalWattsForToilet @Username", new SqlParameter("@Username", username));
                    }
                }
            }
        ...
        public async Task<ActionResult> Tips()
        {
            try
            {
                ...
                {
                    Prompt = "Generate Energy saving only 1 tip for Toilet without numbering and special charactors",
                    Model = Model.DavinciText,
        ...
        public ActionResult UpdateTips(string AffilaiteLink, string ImageAddress, tbl_Refrigilator model)
        {
            ...
            return RedirectToAction("Index", "Waterdispenser");
        }
